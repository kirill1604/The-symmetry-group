...
Основана на условной диффузионной модели `DiffSBDD-Cond`, учитывающей симметрии SE(3) и обученной генерации лигандов с высокой аффинностью к заданному белковому карману. За 2ч 40мин сгенерировать новые лиганды не удалось, поэтому было решено воспользоваться другой моделью.
...
Итоги генерации [тут](./data/sampling_out/REINVENT4/)
...
Также произведена фильтрация и отбор через скоринг REINVENT4  <!-- TODO: Тут нет данных -->

---

## TODO

В дальнейшем предполагается произвести расчеты докинга и молекулярной динамики при появлении требуемых вычислительных ресурсов, вычислить полный профиль аффиности для всех возможных мешеней, а также оптимизировать параметры будущих кандидатов-соединений для минимизации нецелевого связывания и смещенного ингибирования/(обратной)активации (biased inhibition/(reverse)agonism) конкретных молекулярных цепей, следующих за HDAC6 → ...   
Возможно проведение расчетов взаимодействий с G-белками.
